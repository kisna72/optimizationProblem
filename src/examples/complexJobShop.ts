...
export default () => {
    ...
    const operationsFactory = (expandTime, purifyTime, fillTimeA, fillTimeB, capTime, labelTime ) : IComplexOperationUnionList => {
        ...
        //     ]
        // }
        // Expand and Purify can happen in parallel. Both Must happen before the next operation can happen.
    ...
    const operations_a: IComplexOperationUnionList = operationsFactory(10, 30, 10, 20 , 10,8)
    job.addJob({
        id:5,
    ...
    const operations_b: IComplexOperationUnionList = operationsFactory(50, 60, 10, 20 , 10,16)
    job.addJob({
        id:20,
    ...
    const operations_c: IComplexOperationUnionList = operationsFactory(30, 90, 20, 40, 10, 16)
    job.addJob({
        id:30,
        name:"16 OZ Water Bottle",
    ...
    const operations_d: IComplexOperationUnionList = operationsFactory(15, 90, 20, 40, 10, 10,)
    job.addJob({
        id:40,
    ...
    // Incase you wanted to compare with other algorithms:
